...
class AnimeWatchFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        model.getEpisodes().observe(viewLifecycleOwner) { loadedEpisodes ->
            if (loadedEpisodes != null) {
                ...
                if (episodes != null) {
                    // Merge metadata when episodes are loaded
                    mergeEpisodeMetadata(episodes)
        ...
        model.getKitsuEpisodes().observe(viewLifecycleOwner) { i ->
            if (i != null) {
                ...
                // Trigger UI update when Kitsu data arrives
                if (media.anime?.episodes != null) {
                    mergeEpisodeMetadata(media.anime?.episodes!!)
                    reload()
                }
            }
        }
        ...
        model.getFillerEpisodes().observe(viewLifecycleOwner) { i ->
            if (i != null) {
                ...
                // Trigger UI update when Filler data arrives
                if (media.anime?.episodes != null) {
                    mergeEpisodeMetadata(media.anime?.episodes!!)
                    reload()
                }
        ...
        model.getAnifyEpisodes().observe(viewLifecycleOwner) { i ->
            if (i != null) {
                ...
                // Trigger UI update when Anify data arrives
                if (media.anime?.episodes != null) {
                    mergeEpisodeMetadata(media.anime?.episodes!!)
                    reload()
                }
    ...
    private fun mergeEpisodeMetadata(episodes: MutableMap<String, Episode>) {
        episodes.forEach { (key, episode) ->
            // Merge Anify data
            media.anime?.anifyEpisodes?.get(key)?.let { anifyEpisode ->
                episode.desc = anifyEpisode.desc ?: episode.desc
                episode.title = if (MediaNameAdapter.removeEpisodeNumberCompletely(
                        episode.title ?: ""
                    ).isBlank()
                ) anifyEpisode.title ?: episode.title else episode.title
                    ?: anifyEpisode.title ?: episode.title
                episode.thumb = anifyEpisode.thumb ?: episode.thumb
            }
            
            // Merge Filler data
            media.anime?.fillerEpisodes?.get(key)?.let { fillerEpisode ->
                episode.title = fillerEpisode.title ?: episode.title
                episode.filler = fillerEpisode.filler ?: false
            }
            
            // Merge Kitsu data
            media.anime?.kitsuEpisodes?.get(key)?.let { kitsuEpisode ->
                episode.desc = kitsuEpisode.desc ?: episode.desc
                episode.title = if (MediaNameAdapter.removeEpisodeNumberCompletely(
                        episode.title ?: ""
                    ).isBlank()
                ) kitsuEpisode.title ?: episode.title else episode.title
                    ?: kitsuEpisode.title ?: episode.title
                episode.thumb = kitsuEpisode.thumb ?: episode.thumb
            }
        }
    }
    ...
    private fun reload() {
        ...
        if (media.anime!!.episodes != null) {
            // Ensure metadata is merged before displaying
            mergeEpisodeMetadata(media.anime!!.episodes!!)
